...
export const UserProvider: React.FC<UserProviderProps> = ({ children }) => {
	...
	const createReferralUser = async (telegramUser: TelegramUser, inviterId: string) => {
		try {
			const response = await fetch(`/api/invitation`, {
				...
				body: JSON.stringify({ telegramUser, inviterId }),
			});
			const newReferralUser = await response.json();
			return newReferralUser;
		} catch (error) {
			console.error('Failed to create referral user:', error);
			return null;
		}
	};
	...
	const updateUser = async (
		telegramId: string,
		updatedFields: Partial<TelegramUser>
	) => {
		try {
			const response = await fetch(`/api/userByTg?userId=${telegramId}`, {
				method: 'PATCH',
				headers: {
					'Content-Type': 'application/json',
				},
		...
		try {
			...
			if (useApi || tonPrivateKey == null || address == null) {
				const response = await fetch(
					`/api/menstruation?userId=${userId}&months=${months}`
				);
				if (response.ok) {
					const menstruationData = await response.json();
					dates = menstruationData.map(
			...
			const sortedMenstruationData = dates.sort(
	...
	const fetchFarmingSession = async (userId: string) => {
		try {
			const response = await fetch(`/api/farming?userId=${userId}`);
			if (response.ok) {
	...
	const changeMenstruations = async (
		changes: { date: Date; action: 'add' | 'delete' }[]
	) => {
		if (!user) return;
		const useApi = await isUseApi();
		...
		try {
				...
				changes.forEach((change) => {
					if (change.action === 'add') {
						...
						const index = updatedMenstruations.findIndex(
							(periodDate) =>
								periodDate.toDateString() === change.date.toDateString()
						);
						if (index !== -1) {
			...
			if (useTon) {
				...
				if (!storedAddress) {
					...
					console.log(
						'Changing menstruations | using address: ',
						storedAddress
					);
				}
				...
				const publicKey = derivePublicKey(tonPrivateKey);
				await initBlockchainLogic(storedAddress!, publicKey);
				const result = await updateMonthPeriodData(
					storedAddress!,
					changes,
					tonPrivateKey!
				);
		...
		const initializeUser = async () => {
			if (initData?.user) {
				...
				if (existingUser) {
					...
					const nullOrMissingFields = getNullOrMissingFields(
						existingUser,
						telegramUser
					);
					...
					if (initData?.startParam) {
						...
						const inviterId = initData.startParam;

						const referralUser = await createReferralUser(
							convertInitDataToTelegramUser(initData.user),
							inviterId,
						);
					} else {
